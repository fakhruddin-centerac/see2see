...
class ControllerCatalogTours extends Controller {
	...
	protected function getList() {
		...
		/*cusvip water sports*/
        if (isset($this->request->get['filter_category_id'])) {
			$filter_cidcus = $this->request->get['filter_category_id'];
		} else {
			$filter_cidcus = null;
		}
        /**/
        ...
        /*
            if (isset($this->request->get['filter_category_id']-$filter_cidcus)) 
        */
        if ($filter_cidcus==82)
        {
            $this->response->setOutput($this->load->view('catalog/watersports_list', $data));
        }
       else if($filter_cidcus==71)
        {
            $this->response->setOutput($this->load->view('catalog/deserttour_list', $data));
        } 
        else
        {
	...
	protected function getForm() {
        ...
        if (isset($this->request->post['video'])) {
			...
			$data['video'] = $tours_info['video_url'];
		} else {
			$data['video'] = '';
		}
...
//echo "<pre>";print_r($tours_info);exit;
        ...
        if (isset($this->request->post['top_up'])) {
			$data['top_up'] = $this->request->post['top_up'];
		} elseif (!empty($tours_info)) {
			$data['top_up'] = $tours_info['top_up'];
		} else {
			$data['top_up'] = false;
		}
		...
		if (isset($this->request->post['tours_option'])) {
           ...
           if($filter_cidcus==82)
            {
                $tours_options = $this->model_catalog_tours->getProductOptionsAddWSTE(); //customcreate for particular
            }
             else if($filter_cidcus==71)
                {
                   // $this->response->setOutput($this->load->view('catalog/deserttour_list', $data));
                   $tours_options = $this->model_catalog_tours->getProductOptionsAdddt(); //customcreate for particular
                }
            else
            {
...
//echo "<pre>";print_r($tours_option);exit;
       ...
       /*add custom condition*/ 
        if($filter_cidcus==82)
        {
            $this->response->setOutput($this->load->view('catalog/watersports_form', $data));
        }
        else if($filter_cidcus==71)
                {
                   // $this->response->setOutput($this->load->view('catalog/deserttour_list', $data));
                    $this->response->setOutput($this->load->view('catalog/deserttour_form', $data));
                }
        else
        {
	...
	protected function validateForm() {
        ...
        //$this->error['video'] = sprintf($this->language->get('error_video'));
