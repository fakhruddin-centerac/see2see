              ...
              <div class="row">
                <div class="form-group">
                  ...
                  <script type="text/javascript">                      
                      var between = [];
                      function pushdates(start, end){
                        currentDate = new Date(start); 
                        console.log(currentDate);     
                        while (currentDate <= end) {
                          var fullDate = new Date(currentDate);       
                          between.push(('0' + fullDate.getDate()).slice(-2)+ "-"+('0' + (fullDate.getMonth()+1)).slice(-2)+"-"+fullDate.getFullYear());
                          currentDate.setDate(currentDate.getDate() + 1);
                        } 
                        console.log(between);
                      }
                       function editpushdates(start, end){
                        currentDate = new Date(start); 
                  ...
                  <?php $blackout_row = 1; ?>
                    <div id="blackout-row">
                      ...
                      foreach($hotel_specials as $key=>$hotelSpecial){
                      ?>
                      <div class="row blackdatemargin">
                        <?php if($key==0){ ?>
                        <label class="col-sm-2 control-label" for="input-blocked-dates"><?php echo $entry_black_out_dates?></label>
                        <?php }else{ ?>
                        <label class="col-sm-2 control-label" for="input-blocked-dates"></label>
                        <?php } ?>                        
                        <div class="col-sm-3">
                          <input type="text" name="hotel_special[<?php echo $blackout_row; ?>][balckout_date]"  id="input-name<?php echo $blackout_row; ?>" class="form-control"> 
                        </div>   
                        <?php if($key==0){ ?>                     
                        <div class="col-sm-2 text-left">
                          <button type="button" onclick="addBlackout();" data-toggle="tooltip" title="" class="btn btn-primary" data-original-title="Add Blackout Date"><i class="fa fa-plus-circle"></i></button>
                        </div>
                        <?php } ?>
                      </div> 
                      <?php 
                      $blocked_start = $hotelSpecial['date_start'];
                      $blocked_end   = $hotelSpecial['date_end'];
                      ?>
                      <script type="text/javascript">
                      $(function() {
                          var editcurdate = new Date();
                          var editformatted =   editcurdate.getFullYear()+"/"+(editcurdate.getMonth() + 1) +"/"+editcurdate.getDate();
                          $('input[name="hotel_special[<?php echo $blackout_row; ?>][balckout_date]"]').daterangepicker({
                              locale: {
                                  format: 'YYYY-MM-DD',
                                  separator: " / ",
                              },
                          ...
                          editpushdates('<?php echo $blocked_start; ?>', '<?php echo $blocked_end; ?>'); 
                      });
                      </script>
                      ...
                      $(function() {    
                        ...
                        $('input[name="hotel_roomtype[1][room_options][1][stay_period]"').daterangepicker({
                            ...
                            isInvalidDate:function(date){                              
                              if (jQuery.inArray(date.format('DD-MM-YYYY'), between)!='-1') {
                                  return true;  
                              }                                
                            }
                        });
...
//--></script>
<script type="text/javascript"><!--
var blackout_row = <?php echo $blackout_row + 1; ?>;
function addBlackout(){
  html ='<div class="row blackdatemargin"><label class="col-sm-2 control-label" for="input-blocked-dates"></label><div class="col-sm-3">';
  html +='<input type="text" name="hotel_special['+blackout_row+'][balckout_date]"  id="input-name'+blackout_row+'" class="form-control">'; 
  html +='</div></div>';
  $('#blackout-row').append(html); 
  
  var editcurdate = new Date();
                                             
  var editformatted =   editcurdate.getFullYear()+"/"+(editcurdate.getMonth() + 1) +"/"+editcurdate.getDate();
  $('input[name="hotel_special['+blackout_row+'][balckout_date]"]').daterangepicker({
        locale: {
            format: 'YYYY-MM-DD',
            separator: " / ",
        } ,
        minDate: editformatted,      
    }, 
    function(start, end, label) {                                             
      pushdates(start, end);                                   
...
function addRoomType(){ 
  ...
  var editcurdate = new Date();                                             
  var formatted =   editcurdate.getFullYear()+"/"+(editcurdate.getMonth() + 1) +"/"+editcurdate.getDate();
