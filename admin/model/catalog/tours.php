...
$child_array_prce=array();
$adult_array_prce=array();
					...
					if (isset($tours_option['tours_option_value'])) {
						...
						foreach ($tours_option['tours_option_value'] as $tours_option_value) {
						  if(isset($tours_option_value['time_val']))
                            ...
                            if($tours_option['option_id']==14 && $data['top_up']==1) // Private==16 and top tour 
                             {
                               $adult_price=$tours_option_value['price'];
                               $child_prce=$tours_option_value['child_price']; 
                             }
                           else if($data['tours_category'][0]=='82' && $data['top_up']==1 && $tours_option['option_id']==23) // water sports and top tour and with transfer value
                            {
                                
                                $child_array_prce[$tours_option_value['option_value_id']]=$tours_option_value['child_price'];
                                $adult_array_prce[$tours_option_value['option_value_id']]=$tours_option_value['price'];
     ...
     if(isset($data['top_up']))   
      { 
           if($data['top_up']==1 && $data['tours_category'][0]!='82' ) // normal tours
            {$this->db->query("UPDATE " . DB_PREFIX . "product_option_value SET top_up_child = '" . (float)$child_prce . "',top_up_adults = '" . (float)$adult_price . "' WHERE product_id = '" . (int)$tours_id . "' and option_id='16' ");}
           else if($data['top_up']==1 && $data['tours_category'][0]=='82' ) // water sports
           {
            ...
            foreach($adult_array_prce as $key => $value)   
              {
               ...
               foreach($option_value_idIn as $key1 => $vlaue)      
                {
                     $this->db->query("UPDATE " . DB_PREFIX . "product_option_value SET top_up_adults = '" . (float)$value . "' WHERE product_id = '" . (int)$tours_id . "' and option_id='25' and option_value_id='".$vlaue."' ");
                }  
              } 
...
//oc_product_option_value
     ...
     if($data['top_up']==1 && $data['tours_category'][0]!='82') //only tour    
      { $this->db->query("UPDATE " . DB_PREFIX . "product_option_value SET top_up_child = '" . (float)$child_prce . "',top_up_adults = '" . (float)$adult_price . "' WHERE product_id = '" . (int)$tours_id . "' and option_id='16' "); } //update Private price if top up value is ther
	 else if($data['top_up']==1 && $data['tours_category'][0]=='82' ) // water sports
           {
            //338*4+300
            //option_value_id 66 = 15 min
...
//		$tours_option_query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "product_option` po LEFT JOIN `" . DB_PREFIX . "option` o ON (po.option_id = o.option_id) LEFT JOIN `" . DB_PREFIX . "option_description` od ON (o.option_id = od.option_id) WHERE  od.language_id = '" . (int)$this->config->get('config_language_id') . "'");
   /// Oc_option table inner join oc_option_description
   $tours_option_query = $this->db->query("SELECT * FROM  `" . DB_PREFIX . "option` o  LEFT JOIN `" . DB_PREFIX . "option_description` od ON (o.option_id = od.option_id) WHERE  od.language_id = '" . (int)$this->config->get('config_language_id') . "' AND o.option_tp='DT' ");
		//echo "<pre>";print_r($tours_option_query);exit;
        foreach ($tours_option_query->rows as $tours_option) {
			$tours_option_value_data = array();
