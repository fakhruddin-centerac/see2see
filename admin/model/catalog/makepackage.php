			...
			foreach ($data['package_option'] as $package_option) {
				if ($package_option['type'] == 'select' || $package_option['type'] == 'radio' || $package_option['type'] == 'checkbox' || $package_option['type'] == 'image') {
					if (isset($package_option['package_option_value'])) {
						$this->db->query("INSERT INTO " . DB_PREFIX . "product_option SET product_option_id = '" . (int)$package_option['package_option_id'] . "', product_id = '" . (int)$package_id . "', option_id = '" . (int)$package_option['option_id'] . "', required = '" . (int)$package_option['required'] . "'");

						$package_option_id = $this->db->getLastId();

						foreach ($package_option['package_option_value'] as $package_option_value) {
							//time_val = '".$package_option_value['time_val'] ."',price_type = '".$package_option_value['price_type'] ."'
                            if(isset($package_option_value['time_val']))
                            {
                                $timeval = $package_option_value['time_val'];
                            }
                            else
                            {
                                $timeval ='';
                            }
                            if(isset($package_option_value['price_type']))
                            {
                                $pricetype = $package_option_value['price_type'];
                            }
                            else
                            {
                                $pricetype = '';
                            }
                           if($package_option['option_id']==14 && $data['top_up']==1) // Private==16 and top tour 
                             {
                               $adult_price=$package_option_value['price'];
                               $child_prce=$package_option_value['child_price']; 
                             }
                             else if($data['package_category'][0]=='82' && $data['top_up']==1 && $package_option['option_id']==23) // water sports and top tour and with transfer value
                            {
                                
                                $child_array_prce[$package_option_value['option_value_id']]=$package_option_value['child_price'];
                                $adult_array_prce[$package_option_value['option_value_id']]=$package_option_value['price'];
                                
                            }  
                            $this->db->query("INSERT INTO " . DB_PREFIX . "product_option_value SET product_option_value_id = '" . (int)$package_option_value['package_option_value_id'] . "', product_option_id = '" . (int)$package_option_id . "', product_id = '" . (int)$package_id . "', option_id = '" . (int)$package_option['option_id'] . "', option_value_id = '" . (int)$package_option_value['option_value_id'] . "', quantity = '" . (int)$package_option_value['quantity'] . "', subtract = '" . (int)$package_option_value['subtract'] . "', price = '" . (float)$package_option_value['price'] . "',  child_price = '" . (float)$package_option_value['child_price'] . "', price_prefix = '" . $this->db->escape($package_option_value['price_prefix']) . "', points = '" . (int)$package_option_value['points'] . "', points_prefix = '" . $this->db->escape($package_option_value['points_prefix']) . "', weight = '" . (float)$package_option_value['weight'] . "', weight_prefix = '" . $this->db->escape($package_option_value['weight_prefix']) . "', time_val = '". $timeval ."',price_type = '". $pricetype ."' ");
						}
					}
				} else {
					$this->db->query("INSERT INTO " . DB_PREFIX . "product_option SET product_option_id = '" . (int)$package_option['package_option_id'] . "', product_id = '" . (int)$package_id . "', option_id = '" . (int)$package_option['option_id'] . "', value = '" . $this->db->escape($package_option['value']) . "', required = '" . (int)$package_option['required'] . "'");
				}
			}
		}
