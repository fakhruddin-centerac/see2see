				...
				foreach ($option['product_option_value'] as $option_value) {
					...
					if (!$option_value['subtract'] || ($option_value['quantity'] > 0)) {
						if ((($this->config->get('config_customer_price') && $this->customer->isLogged()) || !$this->config->get('config_customer_price')) && (float)$option_value['price']) {
							$price = $this->currency->format($this->tax->calculate(preg_replace( '/[^.\d]/', '', $option_value['price']), $product_info['tax_class_id'], $this->config->get('config_tax') ? 'P' : false), $this->session->data['currency']);
						} else {
							$price = false;
						}
						...
						$product_option_value_data[] = array(
                            ...
                            'child_price'             => $this->currency->format($this->tax->calculate($option_value['child_price'], $product_info['tax_class_id'], $this->config->get('config_tax') ? 'P' : false), $this->session->data['currency']),
                            /*'top_up_child'            => round($option_value['top_up_child']),
                            'top_up_adults'           => round($option_value['top_up_adults']),*/
                            'top_up_child'            => $this->currency->format($this->tax->calculate($option_value['top_up_child'], $product_info['tax_class_id'], $this->config->get('config_tax') ? 'P' : false), $this->session->data['currency']),
                            'top_up_adults'           => $this->currency->format($this->tax->calculate($option_value['top_up_adults'], $product_info['tax_class_id'], $this->config->get('config_tax') ? 'P' : false), $this->session->data['currency']),
                            /*cutom add*/
                            /*'orignalprice'            => round($option_value['price']),*/
                            'orignalprice'            => $this->currency->format($this->tax->calculate(preg_replace( '/[^.\d]/', '', $option_value['price']), $product_info['tax_class_id'], $this->config->get('config_tax') ? 'P' : false), $this->session->data['currency']),
...
//echo "<pre>";print_r($results);exit;
            ...
            else
            {
			     if($category_id==82)
                 {
                    $this->response->setOutput($this->load->view('product/waterspotrs', $data));
                 }
                 else
                 {
                    //echo "<pre>";print_r($data);exit;
