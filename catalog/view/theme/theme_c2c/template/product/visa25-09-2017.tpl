...
function removeapplicanbut(rebutval)
{
    ...
    numberofapp = $('input[name="number_of_applicant"');
    if(rebutminus<numberofapp)
    {
        $('.applicantno-'+rebutval).remove();
    }
    else
    {
        $('.applicantno-'+rebutval).remove();
    ...
    for(i=2;i<=$(".vipinCls").length;i++)
       {
          if(val=='Other')
             ...
             {
              //  alert($(".visaC_"+i+" > div > ul >li:last").css('display'));
...
function checkcunrtybound(country_selectsing)
{
 ...
 function nextTab(elem) 
 {
     ...
     applhtml ='';
     relationtype = $('.personrelationshipvisa .cs-placeholder').text();
