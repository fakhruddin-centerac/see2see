...
class ModelCatalogHotels extends Model {
    ...
    public function getalldataBycategorywise($tp)
    {
        ...
        foreach($catresult as $Result)
        {
            ...
            $porductquery     = $this->db->query("SELECT p.image,pd.description,pd.product_id,pd.name,ptc.category_id  FROM " . DB_PREFIX . "product_to_category ptc  LEFT JOIN ". DB_PREFIX ."product_description pd ON(ptc.product_id=pd.product_id) LEFT JOIN ". DB_PREFIX ."product p ON (p.product_id=pd.product_id) WHERE ptc.category_id='".$Result['category_id']."'  ");
    ...
    {
        $query = $this->db->query("SELECT max(id) as id from " . DB_PREFIX . "applicantinfo ");
        return $query->rows;
    }
    public function addvisainformation($podata){
	...
	public function getProduct($product_id) {
		/*$query = $this->db->query("SELECT DISTINCT *, pd.name AS name, p.country_flag, p.image, m.name AS manufacturer, (SELECT price FROM " . DB_PREFIX . "product_discount pd2 WHERE pd2.product_id = p.product_id AND pd2.customer_group_id = '" . (int)$this->config->get('config_customer_group_id') . "' AND pd2.quantity = '1' AND ((pd2.date_start = '0000-00-00' OR pd2.date_start < NOW()) AND (pd2.date_end = '0000-00-00' OR pd2.date_end > NOW())) ORDER BY pd2.priority ASC, pd2.price ASC LIMIT 1) AS discount, (SELECT price FROM " . DB_PREFIX . "product_special ps WHERE ps.product_id = p.product_id AND ps.customer_group_id = '" . (int)$this->config->get('config_customer_group_id') . "' AND ((ps.date_start = '0000-00-00' OR ps.date_start < NOW()) AND (ps.date_end = '0000-00-00' OR ps.date_end > NOW())) ORDER BY ps.priority ASC, ps.price ASC LIMIT 1) AS special, (SELECT points FROM " . DB_PREFIX . "product_reward pr WHERE pr.product_id = p.product_id AND pr.customer_group_id = '" . (int)$this->config->get('config_customer_group_id') . "') AS reward, (SELECT ss.name FROM " . DB_PREFIX . "stock_status ss WHERE ss.stock_status_id = p.stock_status_id AND ss.language_id = '" . (int)$this->config->get('config_language_id') . "') AS stock_status, (SELECT wcd.unit FROM " . DB_PREFIX . "weight_class_description wcd WHERE p.weight_class_id = wcd.weight_class_id AND wcd.language_id = '" . (int)$this->config->get('config_language_id') . "') AS weight_class, (SELECT lcd.unit FROM " . DB_PREFIX . "length_class_description lcd WHERE p.length_class_id = lcd.length_class_id AND lcd.language_id = '" . (int)$this->config->get('config_language_id') . "') AS length_class, (SELECT AVG(rating) AS total FROM " . DB_PREFIX . "review r1 WHERE r1.product_id = p.product_id AND r1.status = '1' GROUP BY r1.product_id) AS rating, (SELECT COUNT(*) AS total FROM " . DB_PREFIX . "review r2 WHERE r2.product_id = p.product_id AND r2.status = '1' GROUP BY r2.product_id) AS reviews, p.sort_order FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) LEFT JOIN " . DB_PREFIX . "manufacturer m ON (p.manufacturer_id = m.manufacturer_id) WHERE p.product_id = '" . (int)$product_id . "' AND pd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND p.status = '1' AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "'");*/
	...
	public function getProducts($data = array()) {
       ...
       /*$sql = "SELECT DISTINCT(p.product_id),pd.name,p.location,p.price FROM oc_product p LEFT JOIN oc_product_description pd ON pd.product_id=p.product_id 
        LEFT JOIN oc_product_room_option rp ON rp.product_id  = p.product_id
        WHERE ( (p.quantity IN('6','8')) OR (rp.stay_period_start BETWEEN '2017-11-10' AND '2017-11-29') OR (rp.adult_occupancy=2) OR (rp.child_occupancy=1) ) AND (p.product_tp = 'HB')";*/
        ...
        if(isset($data['filter_nation']) && $data['filter_nation']==99){
            $sql .= " (rp.ind_market_price !=0) AND ";
        }else{
            $sql .= " (rp.all_market_price !=0) AND ";
        }

        if(!empty($data['filter_checkin']) && !empty($data['filter_checkout'])){  
           /* $checkin_date = explode('-',$data['filter_checkin']);
            $newCheckinDate = $checkin_date[1].'-'.$checkin_date[0].'-'.$checkin_date[2];
            $checkout_date = explode('-',$data['filter_checkout']);
            $newCheckoutDate = $checkout_date[1].'-'.$checkout_date[0].'-'.$checkout_date[2];*/
        ...
        if(!empty($data['filter_quantity'])){
            ...
            $sql .="(p.quantity IN(".$quantity.")) AND ";
        }

        if(!empty($data['filter_sku'])){
            $sql .="(p.sku LIKE '%".$data['filter_sku']."%') AND ";
        }
        
        if(!empty($data['filter_adult'])){            
            $sql .="(rp.adult_occupancy >='".$data['filter_adult']."') AND ";
        }

        if(!empty($data['filter_child'])){           
            $sql .="(rp.child_occupancy >='".$data['filter_child']."') AND ";
        }

        /* condition */
        /*if( !empty($data['filter_adult']) || !empty($data['filter_child']) ){
             $sql .=" (";
        }     
...
$product_option_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_option po LEFT JOIN `" . DB_PREFIX . "option` o ON (po.option_id = o.option_id) LEFT JOIN " . DB_PREFIX . "option_description od ON (o.option_id = od.option_id) WHERE po.required = '1' AND po.product_id = '" . (int)$product_id . "' AND od.language_id = '" . (int)$this->config->get('config_language_id') . "' ORDER BY o.sort_order");
	...
	public function getTotalProducts($data = array()) {
        ...
        if( !empty($data['filter_sku']) || !empty($data['filter_adult']) || !empty($data['filter_child']) || !empty($data['filter_rooms']) ){
             $sql .=" (";
        }
        ...
        /*if(!empty($data['filter_rooms'])){
            if($where !=''){
                $where .=" OR ";
            }
            $where .=" ( (SELECT count(product_id) as room_count FROM `oc_product_custom` GROUP BY product_id) ='".$data['filter_rooms']."' )";
        }*/
        if(!empty($data['filter_adult'])){
            if($where !=''){
                $where .=" OR ";
    ...
    public function getHotelRoomOptions($hotel_id,$checkin,$checkout){
        ...
        foreach( $room_custom_query->rows as $room_option){
                ...
                if($checkin!='' && $checkout!=""){                   
                    ...
                    $room_option_query  = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_room_option WHERE room_category_id = '". $room_option['product_custom_id'] ."' AND (stay_period_start <= '".date('Y-m-d',strtotime($checkin))."' AND stay_period_end >= '".date('Y-m-d',strtotime($checkout))."') ");
                ...
                foreach($room_option_query->rows as $options){
                    ...
                    $room_option_data[$options['product_room_option_id']]['stay_period_end']    = $options['stay_period_end'];
                    $room_option_data[$options['product_room_option_id']]['all_market_price']   = $this->currency->format($this->tax->calculate($options['all_market_price'], 0, $this->config->get('config_tax') ? 'P' : false), $this->session->data['currency']);
                    $room_option_data[$options['product_room_option_id']]['ind_market_price']   = $this->currency->format($this->tax->calculate($options['ind_market_price'], 0, $this->config->get('config_tax') ? 'P' : false), $this->session->data['currency']);
                    $room_option_data[$options['product_room_option_id']]['all_adult_BB_price'] = $this->currency->format($this->tax->calculate($options['all_adult_BB_price'], 0, $this->config->get('config_tax') ? 'P' : false), $this->session->data['currency']);
                    $room_option_data[$options['product_room_option_id']]['all_child_BB_price'] = $this->currency->format($this->tax->calculate($options['all_child_BB_price'], 0, $this->config->get('config_tax') ? 'P' : false), $this->session->data['currency']);  
                    $room_option_data[$options['product_room_option_id']]['all_adult_HB_price'] = $this->currency->format($this->tax->calculate($options['all_adult_HB_price'], 0, $this->config->get('config_tax') ? 'P' : false), $this->session->data['currency']); 
                    $room_option_data[$options['product_room_option_id']]['all_child_HB_price'] = $this->currency->format($this->tax->calculate($options['all_child_HB_price'], 0, $this->config->get('config_tax') ? 'P' : false), $this->session->data['currency']); 
                    $room_option_data[$options['product_room_option_id']]['all_adult_FB_price'] = $this->currency->format($this->tax->calculate($options['all_adult_FB_price'], 0, $this->config->get('config_tax') ? 'P' : false), $this->session->data['currency']); 
                    $room_option_data[$options['product_room_option_id']]['all_child_FB_price'] = $this->currency->format($this->tax->calculate($options['all_child_FB_price'], 0, $this->config->get('config_tax') ? 'P' : false), $this->session->data['currency']);  
                    $room_option_data[$options['product_room_option_id']]['ind_adult_BB_price'] = $this->currency->format($this->tax->calculate($options['ind_adult_BB_price'], 0, $this->config->get('config_tax') ? 'P' : false), $this->session->data['currency']); 
                    $room_option_data[$options['product_room_option_id']]['ind_child_BB_price'] = $this->currency->format($this->tax->calculate($options['ind_child_BB_price'], 0, $this->config->get('config_tax') ? 'P' : false), $this->session->data['currency']);
                    $room_option_data[$options['product_room_option_id']]['ind_adult_HB_price'] = $this->currency->format($this->tax->calculate($options['ind_adult_HB_price'], 0, $this->config->get('config_tax') ? 'P' : false), $this->session->data['currency']); 
                    $room_option_data[$options['product_room_option_id']]['ind_child_HB_price'] = $this->currency->format($this->tax->calculate($options['ind_child_HB_price'], 0, $this->config->get('config_tax') ? 'P' : false), $this->session->data['currency']);
                    $room_option_data[$options['product_room_option_id']]['ind_adult_FB_price'] = $this->currency->format($this->tax->calculate($options['ind_adult_FB_price'], 0, $this->config->get('config_tax') ? 'P' : false), $this->session->data['currency']); 
                    $room_option_data[$options['product_room_option_id']]['ind_child_FB_price'] = $this->currency->format($this->tax->calculate($options['ind_child_FB_price'], 0, $this->config->get('config_tax') ? 'P' : false), $this->session->data['currency']);
                    ...
                    $room_option_data[$options['product_room_option_id']]['ind_discount_percentage']= $options['ind_discount_percentage'];
                    $room_option_data[$options['product_room_option_id']]['all_discount_percentage']= $options['all_discount_percentage'];
   ...
   /* function storeRoomOptions($optiondata){
    ...
    function storeRoomOptions($optiondata){
        ...
        if(isset($optiondata['discount']) && $optiondata['discount']!=0){
            $discount = $optiondata['discount'];
            $discount_price = ( $optiondata['discount'] / 100 ) * $optiondata['market_price'];            
            $room_total_price = round( $optiondata['market_price'] - $discount_price ); 
        }else{
        ...
        $this->db->query("INSERT INTO " . DB_PREFIX . "hotel_booking SET product_id = '".$optiondata['product_id']."',room_option_id='".$optiondata['room_option_id']."',room_type='".$optiondata['room_type']."',meals_option='".$optiondata['meals_option']."',discount_percentage='".$discount."',room_price ='".$optiondata['market_price']."',rooms_selected='".$optiondata['room_selected']."',no_of_days='".$optiondata['no_of_days']."',total_amount='".$room_total_price."' ");
