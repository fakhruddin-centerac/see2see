<?php
class ModelCatalogStandardPackage extends Model {
	...
	public function getProducts($data = array()) {    
        ...
        $where='';
        $related_list = "'" . implode( "','", $data['filter_list'] ) . "'";
        ...
        LEFT JOIN oc_product_room_option rp ON rp.product_id  = p.product_id WHERE p.status= '1' AND (p.product_id IN (".$related_list.")) AND";
...
$product_option_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_option po LEFT JOIN `" . DB_PREFIX . "option` o ON (po.option_id = o.option_id) LEFT JOIN " . DB_PREFIX . "option_description od ON (o.option_id = od.option_id) WHERE po.required = '1' AND po.product_id = '" . (int)$product_id . "' AND od.language_id = '" . (int)$this->config->get('config_language_id') . "' ORDER BY o.sort_order");
   ...
   /* function storeRoomOptions($optiondata){
        ...
        discount_percentage='".$optiondata['discount']."',room_price ='".$optiondata['market_price']."',rooms_selected='".$optiondata['room_selected']."',total_amount='".$room_total_price."' ");
    }*/
    function storeRoomOptions($optiondata){
    ...
    function storeTours($optiondata){
    	$this->db->query("INSERT INTO " . DB_PREFIX . "standard_tours SET tour_ids = '".$optiondata['tour_ids']."',tour_amount='".$optiondata['tour_amount']."' ");
    	return $this->db->getLastId();
    }
   ...
   function updateRoomOption($order_id,$roomIds){
    ...
    function updateStandardTours($order_id,$tourId){
    	$this->db->query("UPDATE " . DB_PREFIX . "standard_tours SET order_id = '" . (int)$standard_tour_id . "' WHERE order_transport_id = '" . (int)$tourId . "'");    	
    }
